...
{
    //MASS_Ca40 = 37.25 GeV
	TLorentzVector aInVec(pIn.Px()*40,pIn.Py()*40,pIn.Pz()*40,sqrt(pIn.Px()*40*pIn.Px()*40 + pIn.Py()*40*pIn.Py()*40 + pIn.Pz()*40*pIn.Pz()*40 + 37.25*37.25) );
	double method_L = 0;
	...
	double p_Aminus = (37.25*37.25 + p_TAsquared) / p_Aplus;
...
while (tree_reader.Next()) {
	...
	*/
	...
	for(int imc=0;imc<mc_px_array.GetSize();imc++){
        ...
        TVector3 mctrk_test(static_cast<double>(mc_px_array[imc]), 
               static_cast<double>(mc_py_array[imc]), 
               static_cast<double>(mc_pz_array[imc]));
	...
	cout << "this is working!! energy: " << pbeam.E() << endl;
    ...
    TLorentzVector scatREC_MCphi(0,0,0,0);
    TLorentzVector scatClusEREC_MCphi(0,0,0,0);
	...
	for(int itrk=0;itrk<reco_pz_array.GetSize();itrk++){
		...
		if(trk.Mag()>maxP){
            ...
            scatREC_MCphi.SetVectM(mctrk_test,MASS_ELECTRON);

			//use emcal energy to define 4 vector
			double p_MCphi = sqrt(clusEnergy*clusEnergy- MASS_ELECTRON*MASS_ELECTRON );
			double eta_MCphi=scatREC_MCphi.Eta();
			double phi_MCphi=scatREC_MCphi.Phi();
			cout << "phi = " << phi_MCphi << endl;
			double pt_MCphi = TMath::Sin(scatREC_MCphi.Theta())*p;
			scatClusEREC_MCphi.SetPtEtaPhiM(pt_MCphi,eta_MCphi,phi_MCphi,MASS_ELECTRON);
		}
	}
